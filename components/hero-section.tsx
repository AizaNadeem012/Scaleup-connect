...
export default function HeroSection() {
  ...
  return (
    <section
      id="home"
      className="relative min-h-screen flex items-center overflow-hidden pt-12 sm:pt-16 md:pt-20 pb-6 sm:pb-8 md:pb-12 w-full"
    >
      ...
      <div className="absolute inset-0 -z-5">
        <div className="absolute top-10 left-0 w-32 h-32 sm:w-40 sm:h-40 md:w-60 md:h-60 bg-emerald-400/20 rounded-full mix-blend-overlay blur-3xl"></div>
        <div className="absolute bottom-10 right-0 w-40 h-40 sm:w-48 sm:h-48 md:w-72 md:h-72 bg-teal-400/20 rounded-full mix-blend-overlay blur-3xl"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
        ...
        <div className="text-center lg:text-left space-y-4 sm:space-y-5 md:space-y-6">
          ...
          >
            <h2 className="block text-base sm:text-lg md:text-xl font-light tracking-[0.08em] mb-2">
              Empowering{" "}
              <span className="font-semibold tracking-[0.08em] text-emerald-300">
                Careers
              </span>
            </h2>
            <h1 className="mt-2 block text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold relative inline-block leading-[1.1] tracking-tight">
              Enabling Growth
            </h1>
          </motion.div>
          ...
          <motion.p
            ...
            className="text-xs sm:text-sm md:text-base lg:text-lg font-light text-white/95 max-w-xl mx-auto lg:mx-0 leading-relaxed tracking-wide"
          >
          ...
          >
            <Link href="/contact">
              <Button className="bg-white text-[#003728] px-5 sm:px-6 md:px-7 py-2.5 text-sm font-medium rounded-xl hover:bg-emerald-50 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 w-full sm:w-auto">
                Get in Touch
              </Button>
            </Link>
          ...
          <div className="flex flex-col sm:flex-row justify-center lg:justify-start items-center gap-2 sm:gap-3 pt-2 flex-wrap">
            {stats.map((stat, idx) => (
              <motion.div
                key={idx}
